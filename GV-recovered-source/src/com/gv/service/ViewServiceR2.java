...
public class ViewServiceR2 {
    public List<Material> createView(List<Product> m_products, List<Material> m_materials, double saw_bite, double scale) {
        ...
        List<Material> b_materials = null;
        ...
        while (it.hasNext()) {
                    ...
                    if (m_products.size() > 0) {
                        ...
                        if ((m_materials = this.scoreMaterial(m_materials)) == null) {
                            ...
                            while (i < m_materials.size()) {
                                            ...
                                            if (!(m_material.getR_height() < p.getP_height()) && !(m_material.getR_width() < p.getP_width())) {
                                                ...
                                                if (p_count == 0) {
                                                    ...
                                                    if (p_count == 0) {
                                                        ...
                                                        mm_products = this.scoreAreaProductBi1(mm_products, t_flag);
                                                        while (it_t.hasNext()) {
                                                            vt = (Vector)materials_map_index.get(index);
                                                            Vector<ArrayList<Material>> vt_t = new Vector<ArrayList<Material>>();
                                                            if (vt.size() == 0) break;
                                                            for (Object listObj : vt) {
                                                                List<Material> list = (List<Material>)listObj;
                                                                block9: for (Material t_m : list) {
                                                                    int t_i = 0;
                                                                        ...
                                                                        if (t_p.getP_r_count() > 0 && (t_p = this.queryIsUsedProduct(p_products, t_p)).getP_r_count() > t_p.getP_p_count()) {
                                                                            ...
                                                                            if (t_m.getR_width() >= t_p.getP_width() + saw_bite && t_m.getR_height() >= t_p.getP_height() + saw_bite) {
                                                                                ...
                                                                                while (tt_i < mm_products.size()) {
                                                                                    ...
                                                                                    if (t_t_p.getP_r_count() > 0 && t_t_p != t_p) {
                                                                                        t_r_w = t_m.getR_width() - t_t_p.getP_width() - saw_bite;
                                                                                        t_r_t = t_m.getR_height() - t_t_p.getP_height() - saw_bite;
                                                                                        if (0.0 < t_r_w && 0.0 < t_r_t && (t_mod = t_r_w < t_r_t ? t_r_w : t_r_t) < m_mod2 && (t_t_p = this.queryIsUsedProduct(p_products, t_t_p)).getP_r_count() > t_t_p.getP_p_count()) {
                                                        ...
                                                        for (Material material2 : f_materials) {
                                                            if (material2.getM_products() == null) continue;
                                                            f_ps.addAll(CommonTools.cloneProList(material2.getM_products()));
                                                        }
                                                        b_material = CommonTools.cloneScheme(material);
                                                        b_material.setM_products(f_ps);
                                                        b_material.setM_area_used(0.0);
                                                        for (Product product2 : b_material.getM_products()) {
                                                            b_material.setM_area_used(DecimalMath.add(b_material.getM_area_used(), DecimalMath.sub(product2.getP_width(), product2.getP_height())));
                                                        }
                                ...
                                if (scale <= b_material.getM_area_used()) {
                                    int s_i = 0;
                                            ...
                                            while (s_j < b_material.getM_products().size()) {
                                                ...
                                                if (p_j.getP_is_score() == 0) {
                                                    if (p_i.getP_is_dir() == 0) {
                                                        if (p_i.getP_no().equals(p_j.getP_no()) && p_i.getP_is_copy() == p_j.getP_is_copy()) {
                                                            p_i.setP_p_count(p_i.getP_p_count() + 1);
                                                            p_j.setP_is_score(1);
                                                        } else if (p_i.getP_no().equals(p_j.getP_no()) && 1 == p_i.getP_is_copy() + p_j.getP_is_copy()) {
                                                            p_i.setP_p_count(p_i.getP_p_count() + 1);
                                                            p_j.setP_is_score(1);
                                                        }
                                                    } else if (p_i.getP_no().equals(p_j.getP_no())) {
                                                        p_i.setP_p_count(p_i.getP_p_count() + 1);
                                                        p_j.setP_is_score(1);
                                                    }
                                                }
                                    ...
                                    while (z_i < b_material.getM_products().size()) {
                                        ...
                                        if (z_product.getP_is_score() == 0) {
                                            int bs = 0;
                                            for (Product z_p : m_products) {
                                                if (!z_product.getP_no().equals(z_p.getP_no())) continue;
                                ...
                                for (Product p_p : b_material.getM_products()) {
                                    for (Product t_p : m_products) {
                                        if (p_p.getP_is_dir() == 0) {
                                            if (p_p.getP_no().equals(t_p.getP_no()) && p_p.getP_is_copy() == t_p.getP_is_copy()) {
                                                t_p.setP_count(t_p.getP_count() - (double)b_material.getM_u_count());
                                                t_p.setP_r_count(t_p.getP_r_count() - b_material.getM_u_count());
                                                continue;
                                            }
                                            if (!p_p.getP_no().equals(t_p.getP_no()) || 1 != p_p.getP_is_copy() + t_p.getP_is_copy()) continue;
                                            t_p.setP_count(t_p.getP_count() - (double)b_material.getM_u_count());
                                            t_p.setP_r_count(t_p.getP_r_count() - b_material.getM_u_count());
                                            continue;
                                        }
                                        if (!p_p.getP_no().equals(t_p.getP_no())) continue;
                                        t_p.setP_count(t_p.getP_count() - (double)b_material.getM_u_count());
                                        t_p.setP_r_count(t_p.getP_r_count() - b_material.getM_u_count());
                                    }
                                }
                ...
                while (i < b_materials.size()) {
                    if (u_m.getM_is_show() == 0 && ((Material)b_materials.get(i)).getM_is_show() == 0 && u_m != b_materials.get(i) && u_m.getM_name().equals(((Material)b_materials.get(i)).getM_name()) && u_m.getR_height() == ((Material)b_materials.get(i)).getR_height() && ((Material)b_materials.get(i)).getR_width() == u_m.getR_width() && ((Material)b_materials.get(i)).getM_color().equals(u_m.getM_color()) && ((Material)b_materials.get(i)).getM_weight() == u_m.getM_weight() && ((Material)b_materials.get(i)).getM_products().size() == u_m.getM_products().size()) {
                        ...
                        while (j < u_m_products.size()) {
                            if (!u_m_products.get(j).getP_no().equals(((Product)m_t_products.get(j)).getP_no()) || u_m_products.get(j).getM_left() != ((Product)m_t_products.get(j)).getM_left() || u_m_products.get(j).getM_top() != ((Product)m_t_products.get(j)).getM_top() || u_m_products.get(j).getP_width() != ((Product)m_t_products.get(j)).getP_width() || u_m_products.get(j).getP_height() != ((Product)m_t_products.get(j)).getP_height()) break;
                            ++flag2;
                            ++j;
                        }
            ...
            for (Material mm : b_materials) {
                ...
                for (Product ppp : mm.getM_products()) {
                    ...
                    double d = ((Material)m_y2).getR_height() * ((Material)m_y2).getR_width();
                    Object b_m_y = null;
                    b_m_y = s1 >= d ? m_y1 : m_y2;
            ...
            for (Material m_i_material : b_materials) {
                ...
                while (i_i < m_i_material.getM_products().size()) {
                    ...
                    if (i_product.getP_i_score() == 0) {
                        i_product.setP_count(1.0);
                        i_product.setP_i_score(1);
                        int j_i = i_i + 1;
                        while (j_i < m_i_material.getM_products().size()) {
                            if (m_i_material.getM_products().get(j_i).getP_i_score() == 0) {
                                if (i_product.getP_is_dir() == 0) {
                                    if (i_product.getP_name().equals(m_i_material.getM_products().get(j_i).getP_name()) && i_product.getP_height() == m_i_material.getM_products().get(j_i).getP_height() && i_product.getP_width() == m_i_material.getM_products().get(j_i).getP_width() && i_product.getP_is_copy() == m_i_material.getM_products().get(j_i).getP_is_copy()) {
                                        ...
                                        m_i_material.getM_products().get(j_i).setP_i_score(1);
        ...
        switch (score_type) {
            ...
            default: {
                ...
                while (i < l_products.size()) {
                    ...
                    int j = i + 1;
                    while (j < l_products.size()) {
                        if (product.getP_width() * 0.2 * (double)score_type + product.getP_height() * (1.0 - 0.2 * (double)score_type) < l_products.get(j).getP_width() * 0.2 * (double)score_type + l_products.get(j).getP_height() * (1.0 - 0.2 * (double)score_type)) {
                            t_product = l_products.get(j);
                            l_products.set(i, t_product);
                            l_products.set(j, product);
                            product = l_products.get(i);
    ...
    public Product queryIsUsedProduct(List<Product> p_products, Product t_p) {
        ...
        if (t_p.getP_is_dir() == 0) {
            ...
            for (Product p_p : usedProducts) {
                if (!p_p.getP_no().equals(t_p.getP_no()) || p_p.getP_is_copy() != t_p.getP_is_copy()) continue;
                t_p.setP_p_count(t_p.getP_p_count() + 1);
            }
        }
        return t_p;
    ...
    public List<Product> scoreAreaProductBi1(List<Product> products, int score) {
        ...
        while (i < products.size()) {
            ...
            while (j < products.size()) {
                ...
                double c2 = DecimalMath.add(DecimalMath.sub(1.0, DecimalMath.sub(products.get(j).getP_height(), products.get(j).getP_width())), DecimalMath.sub(score, x2 / y2));
                if (c1 < c2) {
                    t_product = products.get(j);
                    products.set(i, t_product);
                    products.set(j, product);
